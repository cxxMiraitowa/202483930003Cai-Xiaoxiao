...
public class GameControl {
	...
	GameControl(GameData gameData, Menu menu) {
		...
		this.menu = menu;
        this.fightInterface = new fightInterface();
	}
	...
	void gameStart() {
		...
		while (true) {
			...
			if (input.length() != 1 || (input.charAt(0) != 'a' && input.charAt(0) != 's' && input.charAt(0) != 'd' && input.charAt(0) != 'w' && input.charAt(0) != '0')) {
	...
	void handleInput(char inC) {
		...
		if (gameData.map[gameData.currentLevel][tX][tY] == 2) {
            ...
            boolean fight = fightInterface.showFightInterface(monsterPower,keyboardInput);

            if (fight) {
                if (gameData.map[gameData.currentLevel][tX][tY] + gameData.heroHealth <= 0) {
                    System.out.print("That monster has " + Integer.toString(monsterPower) + " power, You Lose!!");
                    System.exit(0);
                } else {
                    gameData.heroHealth += gameData.map[gameData.currentLevel][tX][tY];
                    moveHero(tX, tY);
